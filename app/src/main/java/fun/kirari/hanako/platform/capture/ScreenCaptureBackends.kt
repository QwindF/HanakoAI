...
import `fun`.kirari.hanako.platform.capture.projection.MediaProjectionForegroundService
import `fun`.kirari.hanako.platform.capture.projection.ProjectionPermissionActivity
import `fun`.kirari.hanako.platform.capture.projection.ProjectionSessionManager
import `fun`.kirari.hanako.platform.capture.shizuku.ShellScreencap
import `fun`.kirari.hanako.platform.capture.shizuku.ShizukuAuthorizationManager
import `fun`.kirari.hanako.platform.capture.shizuku.ShizukuPermissionActivity
import `fun`.kirari.hanako.platform.capture.shizuku.ShizukuShellScreencap
import `fun`.kirari.hanako.platform.capture.shizuku.ShizukuUserServiceClient
...
internal interface ScreenCaptureBackend {
    ...
    fun requestStart(context: Context, launchMode: CaptureLaunchMode): ScreenCaptureStartResult
...
internal object MediaProjectionCaptureBackend : ScreenCaptureBackend {
    ...
    override fun requestStart(context: Context, launchMode: CaptureLaunchMode): ScreenCaptureStartResult {
        context.startActivity(
            Intent(context, ProjectionPermissionActivity::class.java).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                putExtra(CaptureLaunchContract.EXTRA_LAUNCH_MODE, launchMode.name)
            }
        )
